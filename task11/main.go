...
/* Решение
   1) Считываем две строки. Переводим их в срез строк. Добавляем строки как ключи в хеш таблицу.
	Так делаем для обеих хеш мап.
   2) Вызываем функцию intersection, в которой в цикле for range обходит первую хеш таблицу по ключам
   и если ключ есть во второй таблице добавляет его в срез строк. Это и будет пересечением множеств.
*/
