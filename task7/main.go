...
при работе нескольких горутин. Для этого мы должны использовать sync.Mutex и блокировать
операции над map.
В цикле мы запускаем 100 горутин. В первой функции будет паника при попытке внести изменения
в concurrentMap. Во второй функции доступ к словарю/мапе происходит с использованием блокировок,
таким образом каждая горутина поочередно выполнится как только разблокируется доступ к словарю.

!!! Если расскоментировать 34 строку где происходит обновление concurrentMap - будет паника.
*/
